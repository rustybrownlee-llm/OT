...
// Usage:
//
//	ot-design validate <path>           Validate a device atom, network atom, environment, or process schematic
...
//	ot-design scaffold --process        Write a process schematic skeleton to stdout
...
// It always validates environment.yaml (required) and also validates process.yaml if present.
func validateDirectory(dirPath string) *validate.ValidationResult {
	...
	result := validate.ValidateEnvironment(dirPath)

	processFile := filepath.Join(dirPath, "process.yaml")
	if _, err := os.Stat(processFile); err == nil {
		processResult := validateProcessFile(processFile, dirPath)
		if processResult != nil {
			for _, e := range processResult.Errors {
				result.Add(e)
			}
		}
	}
...
// validateProcessFile validates a process schematic file with cross-reference data.
func validateProcessFile(processFile, envDirPath string) *validate.ValidationResult {
	envFile := filepath.Join(envDirPath, "environment.yaml")
	envDoc, err := validate.LoadFile(envFile)
	if err != nil {
		// Environment load failure already reported by validateDirectory.
		return validate.ValidateProcess(processFile, nil, "")
	}
	designRoot, err := validate.FindDesignRoot(envDirPath)
	if err != nil {
		designRoot = ""
	}
	return validate.ValidateProcess(processFile, envDoc, designRoot)
}
...
func validateFile(path string) *validate.ValidationResult {
	...
	case validate.FileTypeProcess:
		return validateProcessFile(path, filepath.Dir(path))
	default:
		fmt.Fprintf(os.Stderr,
			"ot-design validate: unrecognized YAML file %q: expected top-level 'device:', 'network:', 'environment:', or 'process:' key\n",
			path,
		)
		return nil
	}
...
func runScaffold(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "ot-design scaffold: flag required (--device, --network, --environment, or --process)")
		fmt.Fprintln(os.Stderr, "Usage: ot-design scaffold --device|--network|--environment|--process")
		return 2
	}
	...
	case "--environment":
		...
		return 0
	case "--process":
		fmt.Print(scaffold.Process())
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Save this output to: design/environments/<your-env-id>/process.yaml")
		fmt.Fprintln(os.Stderr, "Example: ot-design scaffold --process > design/environments/my-env/process.yaml")
		return 0
	default:
		fmt.Fprintf(os.Stderr, "ot-design scaffold: unknown flag %q (expected --device, --network, --environment, or --process)\n", args[0])
		return 2
	}
}
...
Validate targets:
  ...
  Process schematic file: ot-design validate design/environments/greenfield-water-mfg/process.yaml
...
When validating a directory, both environment.yaml and process.yaml (if present) are validated.
...
Scaffold output:
  Redirect scaffold output to a file, then edit the placeholder values:
    ...
    ot-design scaffold --process > design/environments/my-new-env/process.yaml

Reference: ADR-009 - Design Layer and Composable Environments (schema v0.1)
Reference: Milestone Beta 0.5 - Process Schematic Schema (schema v0.1)
`)
}
